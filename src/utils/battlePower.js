...
// V_BIG3 학습 결과 (51건, 페어 비율 제약, RMSE 0.28%)
// 누적 정정 이력:
...
export const PHYSICAL_PARAMS = Object.freeze({
  K0: 6.17849644e-1,       // 주스탯 가중치
  K1: 6.06737270e+1,       // 공격력 가중치
  K2: 5.14088007e-1,       // 고댐 가중치
  K_mon: 1.62343581e-1,    // 일몬추+보몬추 가중치 (= K2 × 0.316, 페어 제약)
  D_crit: 2.23769255e+2,   // 크댐 분모
  D_dmg: 1.45252219e-15,   // (최소뎀+최대뎀) 분모
  D_dom: 1.90955624e+2,    // 지배력 분모
  K_geunma: 9.99235770e-4, // 근마효율 계수
  D_pen: 25.0,             // 관통 분모 (고정)
  base: 1.10927917e-22,    // 전체 보정 상수
});

// 마법 직업 파라미터 (V_BIG3 페어 제약, 5건 학습, RMSE 0.13%)
//
// PHYSICAL_PARAMS 대비 K1만 0.97% 상향 (60.6737 → 61.2638).
// 가설: 게임은 무기공격력에 avg(min+max 평균)를 쓰는데 우리는 max를 입력
//   → 물리 K1은 학습 과정에서 (avg/max ≈ 0.990) 비율을 흡수
//   → 마법 속성력은 단일값(=avg-effective)이라 K1을 max-input 비율만큼 역보정해야 정합
// 검증: 실측 캡처 avg/max 평균 0.987과 회귀 0.990이 0.3% 이내 일치.
//       (모델 정밀화로 보정 비율 1.0117 → 1.0097 약간 감소)
export const MAGIC_PARAMS = Object.freeze({
  ...PHYSICAL_PARAMS,
  K1: 6.12638000e+1,       // 마법 속성력 전용 (K1_phys × 1.0097)
});
