 ...
 * 라테일 전투력 계산 모듈
 *
 * 현재 채택: V_BIG32 — 물리 공격력 입력 규칙 최종 정정: 무기공 "표시 범위의 중간값".
 *   자료18/19 실측 범위(93,098~95,231 / 77,963~80,000) 제공으로 판별: 오차 0 역산 공격력이
 *   중간값과 일치 (자료19 는 78,981 vs 중간값 78,981.5 — ±0.5 정확, 오차 +0.0001%).
 *   V_BIG31 의 "min 확정"은 통제캐릭(자료22) 무기공 폭이 7~8이라 min 열과 ceil(중간값) 열이
 *   상수차 4로 완전 동일했던 것 — 통제실험으로는 원리적으로 구분 불가했음 (기울기 43.075 동일).
 *   비반올림 avg·max 열은 기울기 비항상(43.62~43.07 / 44.18~43.07)으로 기각 유지.
 *   방증: 기존4 는 공격력 74,337.5(.5 = 중간값 입력 흔적) 기록에 오차 +0.016% — 애초에 정답 입력.
 *   min/max 폭은 무기별 상이(자료18 2.24%, 자료19 2.55%)라 max 단독 기록으로는 중간값 복원 불가
 *   → 잔여 레거시 물리 캐릭(+0.35~1.8% 편향)은 표시 범위 재캡처 필요.
 *
 *   [V_BIG32 부록] 무기공 "표시" 생성 법칙 자체도 확정 (자료17 + 통제캐릭 6샷, 12관측 12/12 정확):
 *     표시무기공_앞 = floor((기본_앞 + 130) × (1+무기공%)),  표시무기공_뒤 = floor((기본_뒤 + 115) × (1+무기공%))
 *     - 표시는 정렬이 아니라 "위치 대응": 기본 폭 < 15 면 표시가 역순이 됨 (통제캐릭 33,272~33,265 역순의 정체
 *       — 폭 13 < 15). 최종 폭 = (기본 폭 − 15) × (1+%) → 캐릭터마다 min/max 비율이 다른 이유.
 *     - 가산쌍 (130,115) 는 두 캐릭터(자료17 469%, 통제 292%)에서 동일 — %에 안 잡히는 flat 무기공
 *       (계정 공용 장비/칭호 추정, 타 계정 보편성 미확인).
 *     - BP 입력 후보 floor(내부중간값) = 자료17 87,987 / 통제 33,269 — 통제 기울기 검정 통과열(min+4)과
 *       정확 일치, 표시중간값과 ±1 이내 동치. 레거시 복원엔 표시 범위 중간값으로 충분.
 *
 * 이전 채택: V_BIG31 — 3축 통제실험(자료21 깡주스탯 / 자료22 깡무공·속성 / 자료23 깡크댐) 정확 역산으로
...
//
// 입력 규칙 (V_BIG32 정정):
//   - 물리 공격력 = 무기공격력 "표시 범위의 중간값" (예: 93,098~95,231 → 94,164.5)
//     ⚠ max 규칙 기각 유지 (자료22: max 열 선형해 없음). V_BIG31 의 "min" 은 통제캐릭 폭 7~8에서
//       중간값과 상수차 4라 구분 불가였던 것 — 자료18/19 실측 범위(폭 2,133/2,037)로 중간값 확정.
//   - 마법 공격력 = 속성력 (단일값 그대로)
// ============================================================
...
export const PHYSICAL_PARAMS = Object.freeze({
  ...
  K1: 1.871796e+2,         // 공격력 가중치 (물리 = 무기공 "표시범위 중간값", 마법 = 속성력 — 동일 계수)
  K2: 1.1689487,           // 고댐 가중치
...
export const MAGIC_PARAMS = Object.freeze({
  ...
  //   기존 보정은 "물리 공격력에 무기공 max 입력" 규칙 오류를 흡수한 인공물이었음
  //   (게임은 표시범위 중간값 사용 — V_BIG32에서 위 170행 avg 가설이 최종 확증됨).
  K1: 1.871796e+2,
});
