  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        ...
        <p class="text-sm text-slate-600 dark:text-slate-300 leading-snug mb-3">
          이 브라우저에 저장된 캐릭터 <strong>{{ localCharacters.length }}개</strong>가 있습니다.
          계정에 업로드하면 다른 기기에서도 같은 데이터를 사용할 수 있습니다.
        </p>

        <ul class="text-xs space-y-0.5 mb-3 max-h-40 overflow-y-auto bg-slate-50 dark:bg-slate-900/40 rounded-md px-3 py-2 ring-1 ring-slate-200 dark:ring-slate-700">
          <li
          ...
          >
            ...
            <span class="text-slate-400 dark:text-slate-500 ml-2 shrink-0">
              [{{ c.type === 'M' ? '마법' : '물리' }}]
            </span>
          </li>
          <li
            v-if="localCharacters.length > previewLimit"
            class="text-[11px] text-slate-400 dark:text-slate-500 italic pt-1"
          >
            … 외 {{ localCharacters.length - previewLimit }}개
          </li>
        </ul>

        <p class="text-[11px] text-slate-500 dark:text-slate-400 leading-snug mb-3">
          ⓘ 이름이 같은 캐릭터가 이미 계정에 있으면 <code class="text-slate-600 dark:text-slate-400">_local</code> 접미사가 자동으로 붙습니다.
        ...
        <div class="flex items-center gap-2 justify-end">
          <button
            ...
            class="text-xs px-3 py-2 rounded text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition disabled:opacity-40"
          >
            나중에
          </button>
          <button
            ...
            class="text-sm px-4 py-2 rounded bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition disabled:opacity-50"
          >
            {{
